...
export default function HomeView({ subscriptions = [], onManageClick, onExploreClick }) {
    ...
    const nextDelivery = activeSub?.deliveries?.[0]; // We limited this to 1 future delivery in the backend query
    ...
    return (
        <main className="flex-1 p-5 space-y-6 pb-32">
            <header className="flex items-center justify-between mb-2">
                ...
                <div className="h-10 w-10 rounded-full bg-brand-saffron/10 flex items-center justify-center text-brand-saffron border border-brand-saffron/20 capitalize font-bold">
            ...
            <section className="relative">
                ...
                {activeSub && nextDelivery ? (
                    ...
                    <div className="bg-white dark:bg-brand-dark/50 p-6 rounded-xl border border-gray-100 dark:border-gray-800 text-center shadow-sm">
                        <span className="material-symbols-outlined text-4xl text-slate-300 mb-2">restaurant</span>
                        <h3 className="text-slate-900 dark:text-white font-bold">Nothing scheduled</h3>
                        ...
                        <button onClick={onExploreClick} className="px-4 py-2 bg-brand-saffron text-white font-bold rounded-lg shadow-md shadow-brand-saffron/20">
                            See Menu & Plans
                        </button>
                    </div>
                )}
                ...
                {activeSub && (
                    <div className="grid grid-cols-2 gap-3 mt-4">
                        <button onClick={onManageClick} className="flex flex-col items-center justify-center p-3 rounded-xl bg-brand-saffron/10 border border-brand-saffron/30 active:scale-98 transition-transform">
                            <span className="material-symbols-outlined text-brand-saffron mb-1">settings</span>
                        ...
                        <button onClick={onManageClick} className="flex flex-col items-center justify-center p-3 rounded-xl bg-white/50 border border-gray-200 dark:border-gray-700 active:scale-98 transition-transform">
